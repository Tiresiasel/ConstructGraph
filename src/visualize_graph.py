...
from construct_graph.db.neo4j import get_graph
from construct_graph.data.fetchers import fetch_constructs, fetch_relationships, serialize_neo4j_data
from construct_graph.render.page import render_from_jinja, load_template, render_template

# --- 1. CONFIGURATION ---
OUTPUT_HTML_FILE = CONFIG.output_html

# Qdrant vector database configuration
QDRANT_HOST = CONFIG.qdrant.host
QDRANT_PORT = CONFIG.qdrant.port
QDRANT_COLLECTION_NAME = CONFIG.qdrant.collection
...
def fetch_constructs_network_data(graph):
    ...
    try:
        ...
        constructs_result = fetch_constructs(graph)
        ...
        relationships_result = fetch_relationships(graph)
    ...
    <body>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {{
                ...
                // Normalize and collect non-linear shape types from instances within selection window
                function shapesForRel(r) {{
                    const set = new Set();
                    (r.relationship_instances || []).forEach(ri => {{
                        if (!isSelectedAndWithinYear(ri.paper_uid)) return;
                        const raw = (ri.non_linear_type || '').toLowerCase().replace(/\s+/g, '_');
                        if (!raw) return;
                        if (raw.includes('inverted')) {{ set.add('inverted_u'); return; }}
                        if (raw.includes('s')) set.add('s');
                        if (raw.includes('u')) set.add('u');
                    }});
                    return set;
                }}
                ...
                function relationshipMatchesSelection(r) {{
                    ...
                    if (relAll && relAll.checked) {{
                        ...
                        if (!shapes.has('s')) return false;
                    }} else if (relU && relU.checked) {{
                        if (!shapes.has('u')) return false;
                    }} else if (relInvU && relInvU.checked) {{
                        if (!shapes.has('inverted_u')) return false;
                    }}
...
def main():
    ...
    # Generate HTML content with Jinja2 template (Option 2)
    try:
        ...
        # Fallback to legacy inline template replacement if Jinja template missing
