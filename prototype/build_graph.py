...
# --- 1. CONFIGURATION ---
RESET_DATABASE = True  # Set to True to clear database on each run, False for incremental processing
...
PROCESSED_FILES_LOG = "processed_files.json"  # File to track which PDFs have been processed
...
PDF_FOLDER = Path("/Users/tiresias/Documents/PhD/project3/scripts/sample")

# --- 1.0 LOGGING CONFIGURATION ---
def setup_logging():
    """Setup comprehensive logging for the entire build process."""
    ...
    # File handler for LLM extraction logs (INFO level and above)
    llm_logs_handler = RotatingFileHandler(
        logs_dir / "llm_extraction.log",
        maxBytes=10*1024*1024,  # 10MB
        backupCount=5,
        encoding='utf-8'
    )
    llm_logs_handler.setLevel(logging.INFO)
    llm_logs_handler.setFormatter(detailed_formatter)
    logger.addHandler(llm_logs_handler)
    
    # File handler for entity resolution logs (INFO level and above)
    entity_logs_handler = RotatingFileHandler(
        logs_dir / "entity_resolution.log",
        maxBytes=10*1024*1024,  # 10MB
        backupCount=5,
        encoding='utf-8'
    )
    entity_logs_handler.setLevel(logging.INFO)
    entity_logs_handler.setFormatter(detailed_formatter)
    logger.addHandler(entity_logs_handler)
    
    # File handler for graph population logs (INFO level and above)
    graph_logs_handler = RotatingFileHandler(
        logs_dir / "graph_population.log",
        maxBytes=10*1024*1024,  # 10MB
        backupCount=5,
        encoding='utf-8'
    )
    graph_logs_handler.setLevel(logging.INFO)
    graph_logs_handler.setFormatter(detailed_formatter)
    logger.addHandler(graph_logs_handler)
    
    # File handler for JSON output logs (INFO level and above) - NEW!
    json_logs_handler = RotatingFileHandler(
        logs_dir / "json_outputs.log",
        maxBytes=20*1024*1024,  # 20MB for JSON data
        backupCount=3,
        encoding='utf-8'
    )
    json_logs_handler.setLevel(logging.INFO)
    json_logs_handler.setFormatter(detailed_formatter)
    logger.addHandler(json_logs_handler)
    
    # Create specific loggers for different components
    llm_logger = logging.getLogger('llm_extraction')
    entity_logger = logging.getLogger('entity_resolution')
    graph_logger = logging.getLogger('graph_population')
    vector_logger = logging.getLogger('vector_database')
    json_logger = logging.getLogger('json_outputs')  # NEW!
    ...
    json_logger.setLevel(logging.DEBUG)  # NEW!
...
def setup_vector_database():
    """Initialize Qdrant vector database and collections with 1024-dim vectors.
    ...
    try:
        ...
        if RESET_DATABASE and QDRANT_COLLECTION_NAME in existing:
            try:
                client.recreate_collection(
                    collection_name=QDRANT_COLLECTION_NAME,
                    vectors_config=VectorParams(size=1024, distance=Distance.COSINE)
                )
                print(f"Recreated Qdrant collection (reset): {QDRANT_COLLECTION_NAME}")
            except Exception:
                ...
                print(f"Dropped and created Qdrant collection (reset): {QDRANT_COLLECTION_NAME}")
        elif QDRANT_COLLECTION_NAME not in existing:
            client.create_collection(
                collection_name=QDRANT_COLLECTION_NAME,
                vectors_config=VectorParams(size=1024, distance=Distance.COSINE)
            )
...
def generate_embedding(text: str) -> list:
    ...
    try:
        ...
        resp = requests.post(f"{OLLAMA_BASE_URL}/api/embed",  # Use correct endpoint
                           json=payload, timeout=30)
        resp.raise_for_status()
        data = resp.json()
        
        # The /api/embed endpoint returns embeddings in a different format
        embeddings = data.get("embeddings")
        if not embeddings or not isinstance(embeddings, list) or len(embeddings) == 0:
            raise RuntimeError(f"Empty embeddings from Ollama model {MODEL_NAME}")
        
        # Return the first (and only) embedding
        return embeddings[0]
        
    except Exception as e:
        print(f"Error generating embedding with Ollama bge-m3: {e}")
        ...
        try:
            ...
            # Return a default embedding if both models fail
...
def process_pdf_concurrent(pdf_path, vector_db, graph):
    ...
    try:
        ...
        if text:
            ...
            extracted_data = get_info_from_llm(text, pdf_path.name)
            if extracted_data:
                logger.info(f"LLM extraction successful for {pdf_path.name}")

                # Persist artifacts for traceability under output/<hash8>/
                try:
                    # Use md5 hash of absolute path + mtime as stable id seed
                    h = hashlib.md5(str(pdf_path.resolve()).encode('utf-8')).hexdigest()[:16]
                    ...
                    # Copy the source PDF for reference (do not move to avoid breaking upstream paths)
                    try:
                        shutil.copy2(str(pdf_path), str(run_dir / pdf_path.name))
                    except Exception as copy_err:
                        logger.warning(f"Failed to copy PDF artifact: {copy_err}")
                except Exception as art_err:
                    logger.warning(f"Failed to persist output artifacts: {art_err}")
                
                # Intra-paper deduplication
                logger.info(f"Starting intra-paper deduplication for {pdf_path.name}")
                extracted_data = deduplicate_constructs_in_paper(extracted_data)
                logger.info(f"Intra-paper deduplication completed for {pdf_path.name}")
                
                # Step 1: Populate graph
                logger.info(f"Starting graph population for {pdf_path.name}")
                populate_graph_blueprint_compliant(graph, extracted_data, pdf_path.name, vector_db)
                logger.info(f"Graph population completed for {pdf_path.name}")
                
                # Step 2: Entity resolution
                if vector_db:
                    logger.info(f"Starting entity resolution for {pdf_path.name}")
                    resolve_new_constructs_enhanced(graph, extracted_data, pdf_path.name, vector_db)
                    logger.info(f"Entity resolution completed for {pdf_path.name}")
                else:
                    logger.warning(f"Vector database not available, skipping entity resolution for {pdf_path.name}")
...
{text}
    ...
    try:
        ...
        # Log response details with robust None handling
        ...
        safe_len = len(response_content) if isinstance(response_content, str) else 0
        logger.info(f"Response received, length: {safe_len} characters")
        ...
        try:
            ...
            if isinstance(parsed_data, dict):
                ...
                logger.info(f"  - Constructs: {constructs_count}")
                logger.info(f"  - Relationships: {relationships_count}")
                logger.info(f"  - Measurements: {measurements_count}")
                logger.info(f"  - Theories: {theories_count}")
                
                # Log paper metadata if available
                if 'paper_metadata' in parsed_data:
                    metadata = parsed_data['paper_metadata']
                    logger.info(f"Paper metadata extracted:")
                    ...
                    logger.info(f"  - Research Type: {metadata.get('research_type', 'N/A')}")
                
                # Log detailed construct information
                if constructs_count > 0:
                    logger.debug("Constructs extracted:")
                    for i, construct in enumerate(parsed_data['constructs'][:5]):  # Log first 5
                        logger.debug(f"  {i+1}. {construct.get('term', 'N/A')} - {len(construct.get('definition', ''))} chars")
                    if constructs_count > 5:
                        logger.debug(f"  ... and {constructs_count - 5} more constructs")
                
                # Log detailed relationship information
                if relationships_count > 0:
                    logger.debug("Relationships extracted:")
                    for i, rel in enumerate(parsed_data['relationships'][:5]):  # Log first 5
                        logger.debug(f"  {i+1}. {rel.get('subject_term', 'N/A')} -> {rel.get('object_term', 'N/A')} ({rel.get('status', 'N/A')}")
                    if relationships_count > 5:
                        logger.debug(f"  ... and {relationships_count - 5} more relationships")
            
            # NEW: Log the complete JSON output to a separate file for easy review
            json_logger = logging.getLogger('json_outputs')
            json_logger.info(f"\n{'='*80}")
            json_logger.info(f"COMPLETE JSON OUTPUT FOR: {filename}")
            json_logger.info(f"Timestamp: {datetime.now().isoformat()}")
            json_logger.info(f"{'='*80}")
            json_logger.info(f"Raw JSON Response:")
            json_logger.info(json.dumps(parsed_data, indent=2, ensure_ascii=False))
            json_logger.info(f"{'='*80}\n")
...
def populate_graph_blueprint_compliant(graph, data, filename, vector_db):
    ...
    try:
        ...
        for construct in data.get('constructs', []):
            ...
            definition_text = construct.get('definition')
    ...
    for construct in data.get('constructs', []):
        ...
        for result in search_results:
            ...
            if is_same and confidence >= MERGE_CONFIDENCE:
                ...
                # Fully merge: rewire all edges to the shorter_name CC, delete the other CC node
                tx = graph.begin()
                try:
                    query_merge = """
                    MATCH (keep:CanonicalConstruct {preferred_name: $keep_name})
                    MATCH (drop:CanonicalConstruct {preferred_name: $drop_name})
                    // Rewire RelationshipInstance subject/object
                    MATCH (ri:RelationshipInstance)-[hs:HAS_SUBJECT]->(drop)
                    MERGE (ri)-[:HAS_SUBJECT]->(keep)
                    DELETE hs
                    MATCH (ri2:RelationshipInstance)-[ho:HAS_OBJECT]->(drop)
                    MERGE (ri2)-[:HAS_OBJECT]->(keep)
                    DELETE ho
                    // Rewire moderator/mediator/control
                    MATCH (ri3:RelationshipInstance)-[hm:HAS_MODERATOR]->(drop)
                    MERGE (ri3)-[:HAS_MODERATOR]->(keep)
                    DELETE hm
                    MATCH (ri4:RelationshipInstance)-[hmed:HAS_MEDIATOR]->(drop)
                    MERGE (ri4)-[:HAS_MEDIATOR]->(keep)
                    DELETE hmed
                    MATCH (ri5:RelationshipInstance)-[hc:HAS_CONTROL]->(drop)
                    MERGE (ri5)-[:HAS_CONTROL]->(keep)
                    DELETE hc
                    ...
                    WITH keep, drop
                    // Finally delete the dropped CC
                    DETACH DELETE drop
                    // Ensure keep node has the canonical shorter name
                    ...
                    found_similar = True
                    did_merge = True
                    print(f"Merged constructs into canonical '{shorter_name}'")
                    break
                except Exception as e:
...
def main():
    ...
    main_logger.info(f"Loaded {len(processed_files)} previously processed files")
    ...
    all_pdfs, unprocessed_pdfs = get_unprocessed_files(PDF_FOLDER, processed_files)
        ...
        for future in as_completed(future_to_pdf):
            ...
            try:
                ...
                if result:
                    ...
                    main_logger.info(f"Task completed successfully: {pdf_path.name}")
                else:
                    failed_count += 1
                    main_logger.warning(f"Task failed: {pdf_path.name}")
            except Exception as e:
                main_logger.error(f"Exception occurred while processing {pdf_path.name}: {e}")
                failed_count += 1
